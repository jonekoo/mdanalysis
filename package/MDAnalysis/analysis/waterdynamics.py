...
class AngularDistribution(object):
    ...
    :math:`\hat n` parallel to chosen axis (z is the default value). The
    cosine is define as :math:`\cos \theta = \hat u \cdot \hat n`, where
    :math:`\hat u` is OH, HH or dipole vector. It creates a histogram and
    returns a list of lists, see Output_. The AD is also know as Angular
    Probability (AP).
    ...
    :Arguments:
         *universe*
             Universe object
         *selection*
             Selection string to evaluate its angular distribution
             [‘byres name OH2’]
         *bins*
             Number of bins to create the histogram by means of
             numpy.histogram_ [40]
         *axis*
             Axis to create angle with the vector (HH, OH or dipole) and
             calculate cosine theta ['z']. Options: 'x', 'y', 'z'
    ...
    def _getCosTheta(self, universe, selection, axis):
        ...
        while i <= (len(selection)-1):
            ...
            unitOHVector0 = OHVector0/np.linalg.norm(
                OHVector0, axis=1)[:, None]
            unitHHVector0 = HHVector0/np.linalg.norm(
                HHVector0, axis=1)[:, None]
            unitdipVector0 = dipVector0/np.linalg.norm(
                dipVector0, axis=1)[:, None]
    ...
    def _getHistogram(self, universe, selection, bins, axis):
        """
        This function gets a normalized histogram of the cos(theta)
        values. It return a list of list.
        """
    ...
    def run(self, **kwargs):
        ...
        if self.nproc == 1:
            selection = self._selection_serial(self.universe,
                                               self.selection_str)
        else:
            # not implemented yet
            # selection = self._selection_parallel(
            # self.universe,self.selection_str,self.nproc)
            selection = self._selection_serial(self.universe,
                                               self.selection_str)
...
class MeanSquareDisplacement(object):
    r"""
    Function to evaluate the Mean Square Displacement (MSD_). The MSD
    gives the average distance that particles travels. The MSD is given by:
    ...
    :math:`n` is the dimensionality, in this case :math:`n=3`, :math:`D`
    is the diffusion coefficient and :math:`t` is the time.
    ...
    def _getOneDeltaPoint(self, universe, repInd, i, t0, dt):
        """
        Give one point to promediate and get one point of the grapic
        C_vect vs t
    ...
    def _sameMolecTandDT(self, selection, t0d, tf):
        """
        Compare the molecules in the t0d selection and the t0d+dt selection
        and select only the particles that are repeated in both frame.
        This is to consider only the molecules that remains in the
        selection after the dt time has elapsed. The result is a list with
        the indexs of the atoms.
        """
        a = set(selection[t0d])
...
class SurvivalProbability(object):
    ...
    probability for a group of particles to remain in certain region. The
    SP is given by:
     ...
     *selection*
      ...
      define the region/zone where to analize, i.e.: "selection_a" and
      "zone" (see SP examples_ )
     *t0*
      Time where analysis begin
     *tf*
      Time where analysis end
    ...
    def _getMeanOnePoint(self, universe, selection1, selection_str, wint,
        ...
        for frame in range(totalFrames-wint):
            # This "try" is to avoid a division by zero when there is no
            # particles in time t0, this happens in very small selection
            # regions.
            try:
                a = self._getOneDeltaPoint(selection1, totalFrames, frame,
                                           wint)
            except ZeroDivisionError:
                continue
            sumDeltaP += a
